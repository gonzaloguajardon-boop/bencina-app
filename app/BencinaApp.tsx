...
function Calculadora({ today }: { today: number }) {
  ...
  return (
    <div>
      ...
      {L > 0 && deals.length === 0 && (
        <div style={{ textAlign:"center" as const, padding:"30px", color:"#475569" }}>No hay descuentos hoy para calcular.</div>
      )}

      {!L && (
        <div style={{ textAlign:"center" as const, padding:"30px", color:"#334155", fontSize:"13px" }}>
          Ingresa los litros para ver cuánto ahorras con cada tarjeta hoy
        </div>
      )}
    </div>
  );
...
// ─── PRECIOS ──────────────────────────────────────────────────
function Precios() {
  const [estado, setEstado] = useState<"idle"|"ubicando"|"buscando"|"listo"|"error">("idle");
  ...
  async function obtenerPrecios() {
    ...
    try {
      ...
      const comuna = geoData.address?.city || geoData.address?.town || geoData.address?.village || geoData.address?.county || "tu ubicación";
      setCiudad(comuna);
      setEstado("buscando");
      ...
      setEstado("error");
      setRespuesta(e.code === 1 ? "Debes permitir el acceso a tu ubicación." : "No se pudo obtener la información.");
    }
  }

  return (
    <div>
      <div style={{ ...s.greenBox, marginBottom:"16px" }}>
        <div style={{ fontSize:"10px", color:"#4ade80", ...s.mono, marginBottom:"6px" }}>📍 PRECIOS EN TU ZONA</div>
        <div style={{ fontSize:"13px", color:"#94a3b8", marginBottom:"12px" }}>
          Obtén los precios actuales de bencina cerca de ti usando Bencina en Línea.
        </div>
        <button
          onClick={obtenerPrecios}
          disabled={estado === "ubicando" || estado === "buscando"}
          style={{ width:"100%", padding:"10px", borderRadius:"10px", border:"none", background: estado==="ubicando"||estado==="buscando" ? "rgba(74,222,128,0.2)" : "rgba(74,222,128,0.3)", color:"#4ade80", fontSize:"14px", fontWeight:700, cursor:"pointer", fontFamily:"Georgia,serif" }}
        >
          {estado === "ubicando" && "📡 Obteniendo ubicación..."}
          {estado === "buscando" && "🔍 Buscando precios..."}
          {(estado === "idle" || estado === "listo" || estado === "error") && "🔍 Buscar precios cerca de mí"}
        </button>
      </div>
      ...
      {estado === "listo" && respuesta && (
        <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:"14px", padding:"16px" }}>
          <div style={{ fontSize:"10px", color:"#64748b", ...s.mono, marginBottom:"8px" }}>PRECIOS EN {ciudad.toUpperCase()}</div>
          <div style={{ fontSize:"14px", color:"#f1f5f9", lineHeight:1.7, whiteSpace:"pre-wrap" as const }}>{respuesta}</div>
          <div style={{ fontSize:"10px", color:"#334155", marginTop:"12px" }}>Fuente: Bencina en Línea · Actualizado ahora</div>
        </div>
      )}
    ...
    <div style={s.wrap}>
      ...
      <div style={{ padding:"24px 20px 16px", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"4px", marginTop:"16px" }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setView(t.id)} style={s.btn(view===t.id)}>{t.label}</button>
          ))}
        </div>
      </div>
